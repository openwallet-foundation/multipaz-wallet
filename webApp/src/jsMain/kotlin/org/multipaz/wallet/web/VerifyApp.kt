...
val VerifyApp = FC<VerifyAppProps> { props ->
    ...
    useEffectOnce {
        ...
        if (reqId == null || keyB64.isEmpty()) {
            setErrorMessage("Invalid verification link. Missing request ID or encryption key.")
        ...
        scope.launch {
            try {
                ...
                setErrorMessage(e.message ?: "An error occurred while loading the verification request.")
                setStatus(VerifyStatus.ERROR)
            }
        }
    }
    ...
    val handlePresent = {
        ...
        scope.launch {
            try {
                ...
                val jsPromise = js("navigator.credentials.get({ digital: dcRequest })")
                ...
                setErrorMessage(e.message ?: "Failed to present credentials.")
                setStatus(VerifyStatus.ERROR)
            }
        }
    }
        ...
        div {
            ...
            when (status) {
                ...
                VerifyStatus.READY -> {
                    ...
                    svg {
                        val sd: dynamic = this
                        sd.className = "w-16 h-16 text-indigo-500 mb-2 animate-bounce"
                        sd.fill = "none"
                        ...
                        path {
                            ...
                            pd.d = "M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75h-.152c-3.196 0-6.1-1.248-8.25-3.285z"
                        }
                    }
                    h1 {
                ...
                VerifyStatus.ERROR -> {
                    ...
                    button {
                        className = ClassName("w-full py-3.5 bg-slate-800 hover:bg-slate-700 active:bg-slate-900 text-white font-medium rounded-2xl transition-colors duration-150 cursor-pointer mt-4")
                        onClick = {
                            setStatus(VerifyStatus.LOADING)
