    ...
    div {
        ...
        header {
            ...
            div {
                ...
                div {
                    className = ClassName("flex items-center h-full space-x-4")
                    button {
                        className = ClassName("p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-full transition-colors")
                        onClick = { props.onBack() }
                        BackIcon { }
                    }
                    span {
                        className = ClassName("text-xl font-bold text-slate-900")
                        +typeDisplayName
                    }
                }
            }
...
private val h2Section = FC<react.PropsWithChildren> { props ->
    h1 {
        className = ClassName("text-sm font-bold text-slate-400 uppercase tracking-widest px-1")
        +props.children
    }
}

external interface RenderClaimValueProps : Props {
    var claim: org.multipaz.claim.Claim
}
