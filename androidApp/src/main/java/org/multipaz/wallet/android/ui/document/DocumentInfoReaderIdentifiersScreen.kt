...
@Composable
fun DocumentInfoReaderIdentifiersScreen(
    documentId: String,
    ...
    onAddReaderIdentifierClicked: () -> Unit,
    onBackClicked: () -> Unit
) {
    ...
    val readerIdentifiers = documentInfo?.document?.readerIdentifiers.orEmpty()

    val backendInfos = backendReaderTrustManagerModel.trustManagerInfos.collectAsState().value ?: emptyList()
    val userInfos = userReaderTrustManagerModel.trustManagerInfos.collectAsState().value ?: emptyList()
    ...
    val availablePoints = allTrustPoints.filter { point ->
        ...
        ski != null && readerIdentifiers.none { it == ski }
    }
    ...
    Scaffold(
        ...
        topBar = {
            AppMediumTopAppBar(
                title = { Text(stringResource(R.string.document_info_reader_identifiers_screen_title)) },
                navigationIcon = {
                    AppBackButton(onClick = onBackClicked)
                },
                actions = {
                    IconButton(onClick = onAddReaderIdentifierClicked) {
                        Icon(
                            imageVector = Icons.Default.Add,
                            contentDescription = stringResource(
                                R.string.document_info_reader_identifiers_add_manual
                            )
                        )
                    }
                },
        ...
        Column(
            ...
            Note(stringResource(R.string.document_info_reader_identifiers_note))

            FloatingItemList(title = stringResource(R.string.document_info_reader_identifiers_screen_title)) {
                if (readerIdentifiers.isEmpty()) {
                    FloatingItemCenteredText(
                        text = stringResource(R.string.document_info_no_reader_identifiers_set)
                    )
                } else {
                    readerIdentifiers.forEach { identifier ->
                        ...
                        val secondaryText = if (trustPoint == null) {
                            stringResource(R.string.document_info_reader_identifiers_no_trusted_verifier)
                        } else {
                                ...
                                IconButton(onClick = {
                                    ...
                                    if (doc != null) {
                                        coroutineScope.launch {
                                            doc.edit {
                                                this.readerIdentifiers = readerIdentifiers.filter {
                                                    it != identifier
                                                }
                                            }
                                        }
                                    ...
                                    Icon(
                                        ...
                                        contentDescription = stringResource(
                                            R.string.document_info_reader_identifiers_remove
                                        )
                                    )
                                }
                            }
            ...
            FloatingItemList(title = stringResource(R.string.document_info_available_trusted_verifiers)) {
                if (availablePoints.isEmpty()) {
                    FloatingItemCenteredText(
                        text = stringResource(R.string.document_info_no_available_trusted_verifiers)
                    )
                } else {
                        ...
                        FloatingItemText(
                            modifier = Modifier.clickable {
                                if (ski != null) {
                                    ...
                                    if (doc != null) {
                                        coroutineScope.launch {
                                            doc.edit {
                                                this.readerIdentifiers = readerIdentifiers + ski
                                            }
                                        }
                                    }
                                }
                            ...
                            trailingContent = {
                                Icon(
                                    imageVector = Icons.Default.Add,
                                    contentDescription = stringResource(
                                        R.string.document_info_reader_identifiers_add
                                    )
                                )
                            }
                        )
